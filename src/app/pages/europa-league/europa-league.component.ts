...
playQualifyingRound(opponent: Team): void {
  ...
  private generateEvents(
    ...
    if (reds.home) {
      events.push({
        minute: this.uniqueMin(1, 90, usedMins), type: 'red', team: home.name,
        description: `🟥 RED CARD! ${home.name} is down to 10 men!`,
      });
    }
    if (reds.away) {
      events.push({
        minute: this.uniqueMin(1, 90, usedMins), type: 'red', team: away.name,
        description: `🟥 RED CARD! ${away.name} is down to 10 men!`,
      });
    }
    ...
    for (let i = 0; i < extraCount; i++) {
      ...
      events.push({
        minute: this.uniqueMin(1, 90, usedMins), type, team: team.name,
        description: descMap[type](team),
      });
    }
  
    const htHome = events.filter(e =>
      (e.type === 'goal' || e.type === 'penalty_goal') && e.team === home.name && e.minute < 45
    ).length;
    const htAway = events.filter(e =>
      (e.type === 'goal' || e.type === 'penalty_goal') && e.team === away.name && e.minute < 45
    ).length;
    ...
    events.push({
      minute: 91, type: 'info', team: '',
      description: `🏁 Full time: ${home.name} ${homeScoring.goals}–${awayScoring.goals} ${away.name}.`,
    });
  ...
  /** Convierte un conteo bruto de goles en eventos reales (VAR / penalti). */
  private buildScoringEvents(
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (roll < 0.055) {
        events.push({
          minute: min, type: 'var_goal_disallowed', team: team.name,
          description: this.pickVarDisallowedDesc(team.name),
        });
        continue;
      }
      ...
      if (roll < 0.20) {
        events.push({
          minute: Math.max(minFrom, min - 1), type: 'foul', team: opponent.name,
          description: `🚨 Penalty! ${opponent.name} concede a spot-kick after a foul in the area.`,
        });
        events.push({
          minute: min, type: 'penalty_goal', team: team.name,
          description: this.pickPenaltyGoalDesc(team.name),
        });
        goals++;
        continue;
      }
      ...
      events.push({
        minute: min, type: 'goal', team: team.name,
        description: this.pickGoalDesc(team.name, min),
      });
      goals++;
    }
...
private buildHalfEvents(
  home: Team, away: Team,
  homeGoalsRaw: number, awayGoalsRaw: number,
  half: 1 | 2, isFinal: boolean, redEvent?: MatchEvent
  ...
  for (let i = 0; i < extraCount; i++) {
    ...
    const team = useWeak ? weakerTeam : (Math.random() < 0.55 ? dominantTeam : weakerTeam);
    events.push({
      minute: this.uniqueMin(minFrom, minTo, usedMins), type, team: team.name,
      description: descMap[type](team),
    });
  }
  ...
  events.push({
    minute: stoppageMinute, type: 'info', team: '',
    description: `🕒 ${stoppage} minute${stoppage === 1 ? '' : 's'} of stoppage time signalled.`,
  });
  if (Math.random() < 0.35) {
    const lateTeam = Math.random() < 0.5 ? home : away;
    const lateType = pool[Math.floor(Math.random() * pool.length)];
    events.push({
      minute: stoppageMinute, type: lateType, team: lateTeam.name,
      description: descMap[lateType](lateTeam),
    });
  }
...
private playRegulationMatch(home: Team, away: Team, phase: GamePhase, isFinal: boolean): void {
  ...
  const half1Match: MatchResult = {
    home, away,
    homeGoals: half1Built.homeGoals,
    awayGoals: half1Built.awayGoals,
    events: half1Built.events,
    isPlayerMatch: true,
  };
...
resumeSecondHalf(): void {
  ...
  const half2Events = [
    ...half2Built.events,
    {
      minute: 91, type: 'info' as const, team: '',
      description: `🏁 Full time: ${ctx.home.name} ${totalHome}–${totalAway} ${ctx.away.name}.`,
    },
  ];
  half2Events.sort((a, b) => a.minute - b.minute || (a.type === 'info' ? -1 : 1));
  const fullMatch: MatchResult = {
    home: ctx.home, away: ctx.away,
    homeGoals: totalHome, awayGoals: totalAway,
    events: half2Events, isPlayerMatch: true,
  };
  ...
  private simulateExtraTime(
    ...
    const events: MatchEvent[] = [{
      minute: 91, type: 'info', team: '',
      description: `⏱️ Extra time begins! 30 more minutes to separate ${team1.name} and ${team2.name}.`,
    }];
    ...
    if (!existingReds.team1 && redTeam1) {
      events.push({
        minute: this.uniqueMin(92, 119, usedMins), type: 'red', team: team1.name,
        description: `🟥 RED CARD! ${team1.name} is down to 10 men!`,
      });
    }
    if (!existingReds.team2 && redTeam2) {
      events.push({
        minute: this.uniqueMin(92, 119, usedMins), type: 'red', team: team2.name,
        description: `🟥 RED CARD! ${team2.name} is down to 10 men!`,
      });
    }
    ...
    for (let i = 0; i < extraCount; i++) {
      ...
      const team = useWeak ? weakerTeam : (Math.random() < 0.55 ? dominantTeam : weakerTeam);
      events.push({
        minute: this.uniqueMin(92, 119, usedMins), type, team: team.name,
        description: descMap[type](team),
      });
    }
    ...
    const g1First = events.filter(e =>
      (e.type === 'goal' || e.type === 'penalty_goal') && e.team === team1.name && e.minute <= 105
    ).length;
    const g2First = events.filter(e =>
      (e.type === 'goal' || e.type === 'penalty_goal') && e.team === team2.name && e.minute <= 105
    ).length;
  
    events.push({
      minute: 105, type: 'info', team: '',
      description: `⏸️ End of first half of extra time: ${team1.name} ${g1First}-${g2First} ${team2.name}.`,
    });
    events.push({ minute: 106, type: 'info', team: '', description: `▶️ Second half of extra time underway!` });
    const etStoppage = this.rand(1, 3);
    events.push({
      minute: 120, type: 'info', team: '',
      description: `🕒 ${etStoppage} minute${etStoppage === 1 ? '' : 's'} of extra-time stoppage signalled.`,
    });
    events.push({
      minute: 120, type: 'info', team: '',
      description: `🏁 End of extra time: ${team1.name} ${g1}-${g2} ${team2.name}.`,
    });
    events.sort((a, b) => a.minute - b.minute || (a.type === 'info' ? -1 : 1));
  ...
  private startShootout(context: 'qualifying' | 'playoff' | 'knockout'): void {
    ...
    // Misma localía que ET y leg 2: home = team2, away = team1
  ...
  private setupBracketDisplay(): void {
    ...
    this.fullBracket[0] = this.playoffPairs.map(([h, a], i) => ({
    ...
    const sizes = [4, 2, 1]; // QF, SF, Final
    for (let r = 0; r < sizes.length; r++) {
      this.fullBracket[r + 2] = Array.from({ length: sizes[r] }, (_, i) => ({
  ...
  get bracketHalves(): {
    ...
    // Mismo orden; el espejo lo hace CSS (row-reverse)
...
liveScore(teamName: string): number {
  return this.displayedEvents.filter(
    e => (e.type === 'goal' || e.type === 'penalty_goal') && e.team === teamName
  ).length;
}
